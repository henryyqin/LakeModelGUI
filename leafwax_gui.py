...
class SampleApp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (StartPage, PageOne, PageLeafwax):
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button2 = tk.Button(self, text="Run Leafwax Model", font=f,
...
class PageLeafwax(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(self, text="Click to load data from example file", font=f).grid(
            row=rowIdx, column=0, pady=10, sticky="W")
        ...
        tk.Label(self, text="Current File Uploaded:", font=f).grid(
            row=rowIdx, column=0, sticky="W")
        self.exampleFileLabel = tk.Label(self, text="No file", font=f)
        self.exampleFileLabel.grid(
            row=rowIdx, column=1, columnspan=2, pady=10, sticky="W")
        rowIdx += 2

        # Allows user to upload their own .txt data.
        tk.Label(self, text="Click to upload your .txt file:", font=f).grid(
            row=rowIdx, column=0, pady=10, sticky="W")
        graphButton = tk.Button(self, text="Upload .txt File", font=f,
                                command=self.uploadTxt)
        ...
        tk.Button(self, text="Run Leafwax Model", command=self.run_leafwax_model).grid(
            row=rowIdx, column=0, sticky="W")
        rowIdx += 1
        tk.Button(self, text="Generate Graph of Leafwax Proxy Data", command=self.generate_graph).grid(
            row=rowIdx, column=0, sticky="W")
        rowIdx += 1
        tk.Button(self, text="Save Graph Data as .csv", command=self.download_leafwax_data).grid(
            row=rowIdx, column=0, sticky="W")
        rowIdx += 1
        # Return to Start Page
    ...
    def uploadTxt(self, type):
        if type == "sample":
            # Upload example file
            sample_input = 'IsoGSM_dDP_1953_2012.txt'
            self.currentTxtFileLabel.configure(text=sample_input)
