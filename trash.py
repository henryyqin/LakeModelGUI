...
class PageGDGT(tk.Frame):
    ...
    def generate_graph(self):
        ...
        with open("ERA_INTERIM_climatology_Tang_2yr.txt", "r") as data:
            ...
            for line in data:
                line_vals = line.split()
                #what is nspin and why is it causing days=0
                if line_num >= 0: #self.nspin * 12:
                    self.days.append(line_vals[0])
                line_num += 1

        self.days = [int(float(day)) for day in self.days]
        print("length of days and gdgt_proxy")
...
class PageLeafwax(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.a.grid(row=rowIdx, column=1, columnspan=2, pady=10, sticky="W")
        rowIdx +=1
        tk.Label(self, text="End year", font=f).grid(row=rowIdx, column=0, columnspan=2, pady=10, sticky="W")
        self.b = tk.Entry(self)
        self.b.grid(row=rowIdx, column=1, columnspan=2, pady=10, sticky="W")
        rowIdx +=1
        ...
        self.c.grid(row=rowIdx, column=1, columnspan=2, pady=10, sticky="W")
        rowIdx +=1
        tk.Label(self, text="End year", font=f).grid(row=rowIdx, column=0, columnspan=2, pady=10, sticky="W")
        self.d = tk.Entry(self)
        self.d.grid(row=rowIdx, column=1, columnspan=2, pady=10, sticky="W")
        rowIdx +=1
...
# check if years are valid numbers
    ...
    def enter_years(self):
        ...
        if a.isdigit() and b.isdigit() and c.isdigit() and d.isdigit():
            self.start_yr = max(a, c)
            self.end_yr = min(b, d)

        else:
            messagebox.showerror("Make sure all years are digits")
